...
→ 店舗情報（${data.pref}${data.city}の${data.hall}）と「【${eventName}】」という言葉はそれぞれ1回だけ含めること。「初開催」であることに触れ、期待感を出すこと。地名（${data.city}）以外の、その土地の具体的な特徴（駅前・大型店舗の有無など実在するか分からない情報）には触れないこと。2〜3文。` : `■ b2（来店の挨拶＋恒例の取材という紹介。店舗情報と${eventName}という言葉はそれぞれ1回だけ含めること）
実例:
- 「今回は【${eventName}】の取材で${data.pref}${data.city}にある${data.hall}にやって参りました～💨コチラのお店ではすっかり恒例となった取材で、前回も素晴らしい結果を見せていただいたので今回も期待ですね😁」
- 「${data.pref}${data.city}の優良店舗である${data.hall}さんに、今回もお馴染みとなった【${eventName}】取材でお邪魔してきました🎉✨前回に引き続き、どんな景色を見せてくれるのか楽しみです😊」
→ 店舗情報（${data.pref}${data.city}の${data.hall}）と「【${eventName}】」という言葉はそれぞれ1回だけ含めること。「恒例」「お馴染み」など複数回目であることが伝わる表現を使い、前回の結果への期待感を出すこと。地名（${data.city}）以外の、その土地の具体的な特徴には触れないこと。2〜3文。`;
...
→ 「多くのご要望・リクエストを受けてスタートした企画」であることは必ず含め、言い回しだけを変えること。絵文字は🎱や🔮など玉を連想させるものを使い、🤡は使わないこと。1文。` : series === 'ifudodo' ? `■ b3（2026年5月スタートの企画紹介、または今回この店舗で開催することの紹介）
実例:
- 「2026年5月から始まった【${eventName}】💪」
- 「今回、コチラの店舗で開催するのは【${eventName}】💪」
- 「2026年5月にスタートした戦極系新企画【${eventName}】💪」
- 「満を持して開催決定‼️【${eventName}】💪」
- 「今回、この店舗にも【${eventName}】がやって参りました💪」
- 「2026年5月開幕の【${eventName}】、今回はコチラの店舗で開催です💪」
→ 「2026年5月からスタートした企画であること」または「今回この店舗で開催すること」のどちらかの切り口で、言い回しを変えること。絵文字は💪を使うこと。1文。` : series === 'tenchishinmei' ? `■ b3（今夏スタートした新企画の紹介）
実例:
...
→ ${data.lineupTotal ? `総入場数（${data.lineupTotal}名）に必ず触れ、` : ''}朝から来店してくれた方への感謝を伝えること。前回の振り返りや機種名には触れないこと。2文。`;
      } else if (series === 'ifudodo' && data.firstTime) {
        b6Section = `■ b6（初開催）
実例:
...
→ 基本は前回機種(${data.prev})に触れて今回への期待を出すこと。ただし「今回はどの機種が盛り上がっているのか、本当に楽しみですね👍」のように前回機種名を出さずに今回への期待だけを述べるパターンを使ってもよい。${(data.openCount || 0) >= 5 ? `今回で通算${data.openCount}回目の開催という実績があるので、「常連店」として毎回の実績を称える切り口でもよい。` : '今回の開催は通算数回程度のため「常連店」という表現は使わないこと。'}${prevArticle?.over10000Count >= 2 ? `前回は万枚オーバーが${prevArticle.over10000Count}件あったので、その実績に触れてもよい。` : ''}1〜2文。上記のどれかをベースにバリエーションを出すこと。`;
...
${series !== 'denkoisseki' ? `- 合算確率・ボーナス確率・BIG/REGの回数や良し悪しには一切触れないこと（ジャグラー以外では参考にならないデータのため）。「挙動が優秀」などを伝えたい場合は差枚数・出玉・ゲーム数で表現すること
` : ''}
...
【「オーバー」表現の注意】※必ず守ること
- 「〇〇${bodyUnit}オーバー」は"その数値以上"を意味するため、正確な差枚数（例: ${data.t1d}）にそのまま付けないこと（✕「${(data.t1d||'').replace(/\+/,'')}オーバー」）
...
- 毎回少しずつ違う表現・言い回しにすること
- 合算確率・ボーナス確率・BIG/REGの回数や良し悪しには一切触れないこと（ジャグラー以外では参考にならないデータのため）。好調さは差枚数・出玉・ゲーム数で表現すること（例:「全て出玉も申し分ない」）
...
【「オーバー」表現の注意】※必ず守ること
- 「〇〇枚オーバー」は"その数値以上"を意味するため、正確な差枚数（例: ${data.t1d}）にそのまま付けないこと（✕「${data.t1d.replace(/\+/,'')}オーバー」）
- 正確な数値を伝える場合は「${data.t1d.replace(/\+/,'')}枚という結果に」「${data.t1d.replace(/\+/,'')}枚を記録」のように表現すること
- 「オーバー」を使いたい場合は「万枚オーバー」のようにキリの良い基準値に対してのみ使うこと
