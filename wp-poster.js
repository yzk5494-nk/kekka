...
→ 「多くのご要望・リクエストを受けてスタートした企画」であることは必ず含め、言い回しだけを変えること。絵文字は🎱や🔮など玉を連想させるものを使い、🤡は使わないこと。1文。` : `■ b3（${eventName}の形容。慣用句＋イベント名の形）
実例:
...
→ 店舗数（40〜50店舗）・月・表現を少し変えて。1〜2文。`;
      ...
      // 東京喰種はb5で開催回数・前回アピールを扱うためb6は生成しない（空文字を返す）
      const b6Section = series === 'tokyoghoul' ? '' : (data.firstTime ? `■ b6（初開催）
実例:
...
→ 設置台数(${data.total}台)・機種数(${data.cnt}機種)には必ず触れること。機種数が少なく特徴的な構成なら機種名(${data.machineList})を具体的に挙げてもよいが、機種数が多い場合や台数がバラバラな場合は使わないこと。1文。` : '';
      ...
      if (series === 'tokyoghoul') {
        ...
        b13Section = `■ b13（TOP1・2の機種比較ではなく、全体の稼働傾向コメント。機種は東京喰種のみのため機種名には触れない）
...
- 「ランキングのゲーム数を見てもわかるように全体的に高稼働で、東京喰種全体が盛り上がっていました✨️\n中には10,000Gオーバーの台もあり、きっと粘りたくなる要素があったのでしょう🔥🔥」
...
${data.second ? `■ b9（機種構成の紹介。設置台数テーブルの直後に入る）
